...
Stage 5 depth-level target gate is generated by:

```bash
python scripts/06u_generate_depth_level_gate_report.py --config configs/paths.local.yaml
```

The gate may return `go`, `conditional_go`, or `no_go`, but a passing decision
...
Stage 3 review figures are generated by:

```bash
python scripts/06z_generate_depth_level_refinement_review.py --config configs/paths.local.yaml
```

The script writes only review figures and a summary template under:

```text
depth_level_refinement_review_v001/
```

Required figures cover depth-level weak-label candidate vs score by depth,
positive/negative feature distributions, feature-group coefficients,
robustness margin heatmap, permutation margin distribution, confidence
threshold comparison, 3-fold vs 5-fold comparison, and exclude-5700
sensitivity. Figure titles and review templates must state that the labels are
weak-label candidates and that no final labels are created.

Stage 4 refinement gate is generated by:

```bash
python scripts/06aa_generate_depth_level_refinement_gate.py --config configs/paths.local.yaml
```

The gate consumes `depth_level_refinement_report_v001.json`, the prior
`depth_level_baseline_report_v001.json`, and the refinement review summary. It
writes:

```text
depth_level_refinement_gate_report.md
depth_level_refinement_gate_report.json
```

The gate may return `go`, `conditional_go`, or `no_go`. A `go` only means the
depth-level high-confidence weak-label candidate signal is robust enough to
...
Stage 3 manual review figures are generated by:

```bash
python scripts/06ac_generate_depth_level_manual_review_figures.py --config configs/paths.local.yaml
```

The figure script consumes `review_intervals.json`,
`depth_level_labels_v001.npz`, and `depth_level_xsi_features_v001.npz`. It
writes PNG review figures and a figure summary JSON under
