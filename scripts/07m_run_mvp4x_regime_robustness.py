...
from cement_channel.modeling.mvp4x_regime_robustness import (  # noqa: E402
    RegimeRobustnessError,
    run_regime_robustness_from_paths,
)


class RegimeRobustnessCliError(RuntimeError):
    """Raised when the MVP-4X regime robustness CLI cannot run safely."""


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run MVP-4X research-only repeated regime robustness validation."
    )
    parser.add_argument(
        "--paths",
        "--config",
    ...
    parser.add_argument("--stratified-baselines-json", default=None)
...
def main() -> int:
    ...
    try:
        ...
        stratified_json = _resolve_data_path(
            paths,
            "reports",
            args.stratified_baselines_json,
        ...
        output_md = _resolve_data_path(
            ...
            "mvp4x_regime_robustness_v001.md",
        )
        ...
        output_csv = _resolve_data_path(
            ...
            "reports",
            args.output_csv,
            "mvp4x_regime_robustness_v001.csv",
        )
        for path, key in (
            (snapshot_npz, "interim"),
            (waveform_npz, "features"),
            (policy_npz, "interim"),
            (stratified_json, "reports"),
            (output_md, "reports"),
            (output_json, "reports"),
            (output_csv, "reports"),
        ):
        ...
        if args.dry_run:
            print(
                "Dry run: would run MVP-4X regime robustness "
                f"policy={policy_npz} stratified={stratified_json}."
            )
            return 0
        outputs = run_regime_robustness_from_paths(
            snapshot_npz=snapshot_npz,
            waveform_features_npz=waveform_npz,
            policy_npz=policy_npz,
            stratified_baselines_json=stratified_json,
    ...
    print(
        ...
        f"candidate_count={outputs.report['candidate_count']}; "
        f"supported={outputs.report['screening_support']['supported_screening_cohorts']}; "
        f"research_only={outputs.report['research_only']}."
    )
    print(f"Wrote robustness JSON: {output_json}")
    return 0
...
def _resolve_data_path(
    ...
    raise RegimeRobustnessCliError(f"data.{key} is not configured.")
...
def _ensure_path_within(config: dict[str, Any], path: Path, *, key: str) -> None:
    ...
    if root != resolved and root not in resolved.parents:
        raise RegimeRobustnessCliError(f"{resolved} is outside configured data.{key}: {root}")
