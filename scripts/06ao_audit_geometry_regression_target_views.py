...
from cement_channel.evaluation.geometry_regression_target_view_audit import (  # noqa: E402
    audit_geometry_regression_target_views_from_paths,
)


class GeometryRegressionTargetViewCliError(RuntimeError):
    """Raised when target-view audit cannot run safely."""


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Audit existing geometry regression depth-level target views."
    )
    parser.add_argument(
        "--paths",
        "--config",
...
def main() -> int:
    ...
    try:
        ...
        report = audit_geometry_regression_target_views_from_paths(
        ...
        ManifestBuildError,
        GeometryRegressionTargetViewCliError,
        OSError,
        ValueError,
        KeyError,
        FileExistsError,
    ...
    print(
        ...
        f"rows={len(report.view_summaries)}; "
...
def _resolve_interim_path(config: dict[str, Any], override: str | None, filename: str) -> Path:
    ...
    raise GeometryRegressionTargetViewCliError("data.interim is not configured.")
...
def _resolve_report_path(config: dict[str, Any], override: str | None, filename: str) -> Path:
    ...
    raise GeometryRegressionTargetViewCliError("data.reports is not configured.")
...
def _ensure_path_within(
    ...
    *,
    ...
    if not str(root):
        raise GeometryRegressionTargetViewCliError(f"data.{key} is not configured.")
    try:
        path.resolve().relative_to(root)
    except ValueError as exc:
        raise GeometryRegressionTargetViewCliError(
            f"Refusing to {action} target-view audit path outside data.{key}: {path}"
        ) from exc
