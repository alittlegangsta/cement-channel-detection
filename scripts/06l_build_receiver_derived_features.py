...
from cement_channel.features.receiver_derived_features import (  # noqa: E402
    build_receiver_derived_features_from_config,
)


class ReceiverDerivedFeatureCliError(RuntimeError):
    """Raised when receiver-derived feature generation cannot run safely."""


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build MVP-4B-R2 receiver-derived features.")
    parser.add_argument(
        "--paths",
        "--config",
        dest="paths_config",
...
def main() -> int:
    ...
    try:
        ...
        output_md = _resolve_report_path(
            ...
            "receiver_derived_feature_report_v001.md",
        )
        output_json = _resolve_report_path(
            paths,
            args.output_report_json,
            "receiver_derived_feature_report_v001.json",
        )
        ...
        if args.dry_run:
            ...
            from cement_channel.features.receiver_derived_features import (  # noqa: PLC0415
                build_receiver_derived_feature_table,
            )
            from cement_channel.features.receiver_feature_schema import (  # noqa: PLC0415
            ...
            _, report = build_receiver_derived_feature_table(
                basic_arrays=basic_arrays,
                sample_arrays=sample_arrays,
                config=load_receiver_feature_config(args.receiver_config),
                inputs={
            ...
            report = build_receiver_derived_features_from_config(
                basic_features_npz=basic_npz,
                sample_table_npz=sample_npz,
                receiver_config_path=args.receiver_config,
                output_npz=output_npz,
        ...
        ManifestBuildError,
        ReceiverDerivedFeatureCliError,
        OSError,
        ValueError,
        KeyError,
        FileExistsError,
    ...
    print(
        "Receiver-derived features "
        f"errors={len(report.errors)}; "
        f"warnings={len(report.warnings)}; "
        f"raw_features={report.raw_receiver_feature_count}; "
        f"transformed_features={report.transformed_receiver_feature_count}; "
        f"finite_ratio={report.finite_ratio['transformed_receiver_features']}; "
    ...
    if args.dry_run:
        ...
        print(f"Wrote receiver-enhanced sample table: {output_npz}")
...
def _resolve_feature_path(config: dict[str, Any], override: str | None, filename: str) -> Path:
    ...
    raise ReceiverDerivedFeatureCliError("data.features is not configured.")
...
def _resolve_interim_path(config: dict[str, Any], override: str | None, filename: str) -> Path:
    ...
    raise ReceiverDerivedFeatureCliError("data.interim is not configured.")
...
def _resolve_report_path(config: dict[str, Any], override: str | None, filename: str) -> Path:
    ...
    raise ReceiverDerivedFeatureCliError("data.reports is not configured.")
...
def _ensure_path_within(
    ...
    *,
    ...
    if not str(root):
        raise ReceiverDerivedFeatureCliError(f"data.{key} is not configured.")
    try:
        path.resolve().relative_to(root)
    except ValueError as exc:
        raise ReceiverDerivedFeatureCliError(
            f"Refusing to {action} receiver feature path outside data.{key}: {path}"
        ) from exc
