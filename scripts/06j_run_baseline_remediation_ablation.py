...
from cement_channel.training.simple_baseline import (  # noqa: E402
    run_baseline_remediation_ablation_from_config,
    write_baseline_remediation_ablation_outputs,
)


class BaselineRemediationCliError(RuntimeError):
    """Raised when MVP-4B-R baseline remediation ablation cannot run safely."""


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run MVP-4B-R controlled baseline remediation ablations."
    )
    parser.add_argument(
        "--paths",
        "--config",
...
def main() -> int:
    ...
    try:
        ...
        output_md = _resolve_report_path(
            ...
            "baseline_remediation_ablation_v001.md",
        )
        ...
        output_csv = _resolve_report_path(
            paths,
            args.output_csv,
            "baseline_remediation_ablation_v001.csv",
        )
        ...
        report = run_baseline_remediation_ablation_from_config(
    ...
    print(
        "Baseline remediation ablation "
        f"errors={len(report.errors)}; "
        f"warnings={len(report.warnings)}; "
        f"scenarios={report.scenario_count}; "
        f"best_margin={report.best_non_degenerate_margin}; "
        "class_balanced_success="
        f"{report.class_balanced_non_degenerate_above_permutation}; "
        f"no_go_reasons={','.join(report.no_go_reasons)}."
    )
    if args.dry_run:
...
def _resolve_interim_path(config: dict[str, Any], override: str | None, filename: str) -> Path:
    ...
    raise BaselineRemediationCliError("data.interim is not configured.")
...
def _resolve_report_path(config: dict[str, Any], override: str | None, filename: str) -> Path:
    ...
    raise BaselineRemediationCliError("data.reports is not configured.")
...
def _ensure_path_within(
    ...
    *,
    ...
    if not str(root):
        raise BaselineRemediationCliError(f"data.{key} is not configured.")
    try:
        path.resolve().relative_to(root)
    except ValueError as exc:
        raise BaselineRemediationCliError(
            f"Refusing to {action} remediation ablation path outside data.{key}: {path}"
        ) from exc
