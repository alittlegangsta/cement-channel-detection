...
class DepthLevelRefinementReviewCliError(RuntimeError):
    """Raised when depth-level refinement review figures cannot be generated safely."""


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate MVP-4B-R4c depth-level refinement review figures."
    )
    parser.add_argument(
        "--paths",
        "--config",
...
def main() -> int:
    ...
    try:
        ...
        if args.dry_run:
            ...
            review = generate_depth_level_refinement_review_figures(
                refinement_report_json=report_json,
                refinement_csv=prediction_csv,
        ...
        ManifestBuildError,
        DepthLevelRefinementReviewCliError,
        OSError,
        ValueError,
        KeyError,
        FileNotFoundError,
    ...
    if args.dry_run:
        print(
            "Dry run: depth-level refinement review figures would be written "
            f"under {output_dir}."
        )
        return 0
    assert review is not None
    print(
        "Depth-level refinement review figures "
        f"errors={len(review.errors)}; "
        f"warnings={len(review.warnings)}; "
        f"figures={len(review.figures)}; "
        f"preferred_scenario={review.preferred_scenario_id}; "
        f"preferred_feature_group={review.preferred_feature_group}; "
...
def _resolve_report_path(config: dict[str, Any], override: str | None, filename: str) -> Path:
    ...
    raise DepthLevelRefinementReviewCliError("data.reports is not configured.")
...
def _resolve_interim_path(config: dict[str, Any], override: str | None, filename: str) -> Path:
    ...
    raise DepthLevelRefinementReviewCliError("data.interim is not configured.")
...
def _resolve_review_dir(config: dict[str, Any], override: str | None) -> Path:
    ...
    reports = data.get("reports")
    if reports:
        return Path(str(reports)) / "depth_level_refinement_review_v001"
    raise DepthLevelRefinementReviewCliError("data.reports is not configured.")
...
def _ensure_path_within(
    ...
    *,
    ...
    if not str(root):
        raise DepthLevelRefinementReviewCliError(f"data.{key} is not configured.")
    try:
        path.resolve().relative_to(root)
    except ValueError as exc:
        raise DepthLevelRefinementReviewCliError(
            f"Refusing to {action} depth-level refinement review path outside data.{key}: {path}"
        ) from exc
