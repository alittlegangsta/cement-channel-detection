...
from cement_channel.evaluation.mvp4x_regime_error_analysis import (  # noqa: E402
    RegimeErrorAnalysisError,
    run_regime_error_analysis_from_paths,
)


class RegimeErrorAnalysisCliError(RuntimeError):
    """Raised when the MVP-4X error analysis CLI cannot run safely."""


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Analyze MVP-4X regime baseline errors.")
    parser.add_argument(
        "--paths",
        "--config",
        dest="paths_config",
...
def main() -> int:
    ...
    try:
        ...
        stratified_decision = _resolve_data_path(
            paths,
            "reports",
        ...
        output_md = _resolve_data_path(
            ...
            "mvp4x_regime_error_analysis_v001.md",
        )
        ...
        output_csv = _resolve_data_path(
            ...
            "reports",
            args.output_csv,
            "mvp4x_regime_error_analysis_v001.csv",
        )
        for path, key in (
        ...
        if args.dry_run:
            print(
                "Dry run: would analyze MVP-4X regime errors "
                f"robustness={robustness_json} ranking={ranking_json}."
            )
            return 0
        outputs = run_regime_error_analysis_from_paths(
            ...
            stratified_decision_json=stratified_decision,
    ...
    print(
        ...
        f"row_count={len(outputs.report['rows'])}; "
        "calibration_insufficient="
        f"{outputs.report['summary']['absolute_calibration_insufficient']}; "
        f"research_only={outputs.report['research_only']}."
    )
    print(f"Wrote error analysis JSON: {output_json}")
    return 0
...
def _resolve_data_path(
    ...
    raise RegimeErrorAnalysisCliError(f"data.{key} is not configured.")
...
def _ensure_path_within(config: dict[str, Any], path: Path, *, key: str) -> None:
    ...
    if root != resolved and root not in resolved.parents:
        raise RegimeErrorAnalysisCliError(f"{resolved} is outside configured data.{key}: {root}")
