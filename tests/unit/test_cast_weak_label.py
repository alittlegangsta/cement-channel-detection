...
def _label_config() -> dict:
    return {
        "threshold": {
            ...
            "candidate_coverage_blocking_min": 0.0,
...
def test_generate_cast_weak_labels_outputs_plus_and_minus_candidates(tmp_path: Path) -> None:
    ...
    assert arrays["final_label_confidence_plus"][1, 2] == arrays["label_confidence_plus"][1, 2]
    assert "bad_data_confidence_plus" in arrays
    assert "orientation_confidence_on_cast_depth_plus" in arrays
...
def test_bad_data_mask_prevents_extreme_zc_from_becoming_severe(tmp_path: Path) -> None:
    input_npz = tmp_path / "cast_label_input_v001.npz"
    baseline_npz = tmp_path / "cast_zc_baseline_v001.npz"
    zc = np.full((2, 4), 10.0, dtype=np.float32)
    zc[0, 0] = 0.0
    zc[0, 1] = -1.0
    zc[0, 2] = np.nan
    zc[1, 0] = 0.2
    ...
    np.savez_compressed(
        input_npz,
        cast_depth=np.array([100.0, 101.0], dtype=np.float32),
        cast_azimuth_deg=np.arange(4, dtype=np.float32) * 90.0,
        cast_zc=zc,
        relbearing_deg=np.array([0.0, 0.0], dtype=np.float32),
        orientation_confidence=np.array([1.0, 1.0], dtype=np.float32),
        orientation_uncertain=np.array([False, False]),
    )
    np.savez_compressed(
        baseline_npz,
        zc_base=base,
    ...
    assert report.errors == []
    assert arrays["bad_data_mask_plus"][0, 0]
    assert arrays["bad_data_mask_plus"][0, 1]
    assert arrays["bad_data_mask_plus"][0, 2]
    assert arrays["relative_drop_outlier_plus"][1, 0]
    assert arrays["presence_plus"][0, 0] == -1
    assert arrays["presence_plus"][1, 0] == -1
    assert arrays["severity_plus"][1, 0] == -1
    assert arrays["label_confidence_plus"][1, 0] == 0.0


def test_confidence_decomposition_explains_low_orientation(tmp_path: Path) -> None:
    input_npz, baseline_npz = _write_inputs(tmp_path)
    ...
    assert np.isclose(
        report.confidence_components["plus"]["orientation_confidence_on_cast_depth"]["min"],
        0.05,
    )
    assert arrays["zc_strength_confidence_plus"][1, 2] > 0.8
    assert arrays["orientation_confidence_on_cast_depth_plus"][1, 2] == 0.05
    assert arrays["baseline_confidence_plus"][1, 2] == 1.0
    assert arrays["relbearing_valid_confidence_plus"][1, 2] == 1.0
    assert arrays["bad_data_confidence_plus"][1, 2] == 1.0
