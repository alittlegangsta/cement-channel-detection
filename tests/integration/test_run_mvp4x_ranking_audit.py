...
def test_run_ranking_audit_cli_writes_reports(tmp_path: Path) -> None:
    data = tmp_path / "data"
    ...
    config.write_text(
        yaml.safe_dump(
            {
                "robustness": {"n_contiguous_folds": 3, "ridge_alpha": 1.0},
                "ranking_audit": {"permutation_count": 1},
            }
        ),
        encoding="utf-8",
    )
    ...
    result = subprocess.run(
        [
            sys.executable,
            "scripts/07n_run_mvp4x_ranking_audit.py",
            "--paths",
            str(paths),
            "--policy-config",
            str(config),
    ...
    report = json.loads((data / "reports/mvp4x_ranking_audit_v001.json").read_text())
    assert report["research_only"]
    assert report["derived_ordinal_audit_only"]
