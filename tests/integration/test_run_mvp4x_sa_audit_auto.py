...
def test_run_mvp4x_sa_audit_auto_dry_run(tmp_path: Path) -> None:
    ...
    result = subprocess.run(
        [
            sys.executable,
            "scripts/07x_run_mvp4x_sa_audit_auto.py",
            "--paths",
            str(paths_config),
            "--pilot-run-dir",
    ...
    assert "Dry run: would run MVP-4X-SA audit" in result.stdout
