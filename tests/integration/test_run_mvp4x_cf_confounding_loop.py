...
def test_cf_confounding_loop_dry_run(tmp_path) -> None:
    ...
    result = subprocess.run(
        [
            sys.executable,
            "scripts/07g_run_mvp4x_cf_confounding_loop.py",
            "--paths",
            str(paths_config),
            "--dry-run",
        ],
    ...
    assert "Dry run: would run MVP-4X CF loop" in result.stdout
